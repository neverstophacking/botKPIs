...
export async function calcularKpisDesdeDB(codigoTienda, fechaInicio, fechaFin) {
    ...
    const pedidosEncontrados = await prisma.order.count({
        where: {
            storeId: tienda.id,
            dateCreated: {
                gte: fechaInicio,
    ...
    const pedidosValidos = await prisma.order.findMany({
        where: {
            storeId: tienda.id,
            status: {
                in: ESTADOS_VALIDOS,
    ...
    const cantidadPedidosValidos = pedidosValidos.length;
    const ticketPromedio = cantidadPedidosValidos > 0
        ? ventas / cantidadPedidosValidos
        : 0;
    return {
        pedidos_encontrados: pedidosEncontrados,
        pedidos_validos: cantidadPedidosValidos,
...
//# sourceMappingURL=db-kpi.service.js.map