...
function crearRangoFechaTexto(fecha) {
    const [dia, mes, anio] = fecha.split("-").map(Number);
    const base = new Date(anio, mes - 1, dia);
    return crearRangoDia(base);
}
router.get("/todas/comparacion", async (_req, res) => {
    try {
        ...
        const variaciones = {
            ...
            cantidad_porcentaje: calcularVariacion(hoyKpis.cantidad_vendida, ayerKpis.cantidad_vendida),
        ...
        return res.json({
            ...
            periodo: "hoy_vs_ayer",
            fechas: {
                hoy: formatoMexico(hoy),
                ayer: formatoMexico(ayer),
            },
            zona_horaria: ZONA_HORARIA,
            tiempo_consulta_ms: Date.now() -
                inicioMedicion,
            hoy: hoyKpis,
            ayer: ayerKpis,
            variaciones,
            tiendas_hoy: resultadoHoy.tiendas,
            tiendas_ayer: resultadoAyer.tiendas,
        });
    }
    catch (error) {
        console.error(error);
        return res.status(500).json({
            error: "No fue posible realizar la comparación.",
            detalle: error.message,
        });
    }
});
router.get("/todas/productos/hoy", async (_req, res) => {
    try {
        const hoy = obtenerFechaActualMexico();
        const { inicio, fin, } = crearRangoDia(hoy);
        const inicioMedicion = Date.now();
        const productos = await calcularTopProductosDesdeDB(inicio, fin, 5);
        return res.json({
            fuente: "postgresql",
            periodo: "productos_hoy",
            fecha: formatoMexico(hoy),
            zona_horaria: ZONA_HORARIA,
            tiempo_consulta_ms: Date.now() -
                inicioMedicion,
            ...productos,
        ...
        console.error(error);
        return res.status(500).json({
            error: "No fue posible calcular los productos de hoy.",
            detalle: error.message,
        });
    }
});
router.get("/todas/productos/semana", async (_req, res) => {
    try {
        const hoy = obtenerFechaActualMexico();
        ...
        inicioSemana.setDate(hoy.getDate() -
            diasDesdeLunes);
        const inicio = new Date(inicioSemana.getFullYear(), inicioSemana.getMonth(), inicioSemana.getDate(), 0, 0, 0, 0);
        const fin = new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate(), 23, 59, 59, 999);
        const inicioMedicion = Date.now();
        const productos = await calcularTopProductosDesdeDB(inicio, fin, 5);
        return res.json({
            fuente: "postgresql",
            periodo: "productos_semana",
            desde: formatoMexico(inicioSemana),
            hasta: formatoMexico(hoy),
            zona_horaria: ZONA_HORARIA,
            tiempo_consulta_ms: Date.now() -
                inicioMedicion,
            ...productos,
        ...
        console.error(error);
        return res.status(500).json({
            error: "No fue posible calcular los productos de la semana.",
            detalle: error.message,
        });
    }
});
router.get("/todas/productos/mes", async (_req, res) => {
    try {
        const hoy = obtenerFechaActualMexico();
        const inicioMes = new Date(hoy.getFullYear(), hoy.getMonth(), 1, 0, 0, 0, 0);
        const fin = new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate(), 23, 59, 59, 999);
        const inicioMedicion = Date.now();
        const productos = await calcularTopProductosDesdeDB(inicioMes, fin, 5);
        return res.json({
            fuente: "postgresql",
            periodo: "productos_mes",
            desde: formatoMexico(inicioMes),
            hasta: formatoMexico(hoy),
            zona_horaria: ZONA_HORARIA,
            tiempo_consulta_ms: Date.now() -
                inicioMedicion,
        ...
        console.error(error);
        return res.status(500).json({
            error: "No fue posible calcular los productos del mes.",
            detalle: error.message,
        });
    }
});
    ...
    try {
        ...
        if (!fecha) {
            return res.status(400).json({
                error: "Debes proporcionar una fecha.",
                formato: "DD-MM-YYYY",
                ejemplo: `/api/db/kpis/${codigoTienda}?fecha=18-08-2026`,
            });
        }
        ...
        const { inicio, fin, } = crearRangoFechaTexto(fecha);
        const inicioMedicion = Date.now();
        const kpis = await calcularKpisDesdeDB(codigoTienda, inicio, fin);
        ...
        console.error(error);
        return res.status(500).json({
            error: "No fue posible calcular los KPIs.",
            detalle: error.message,
        });
    }
});
router.get("/todas/hoy", async (_req, res) => {
    try {
        const hoy = obtenerFechaActualMexico();
        const { inicio, fin, } = crearRangoDia(hoy);
        const inicioMedicion = Date.now();
        ...
        console.error(error);
        return res.status(500).json({
            error: "No fue posible calcular el consolidado de hoy.",
            detalle: error.message,
        });
    }
});
...
router.get("/:tienda/hoy", async (req, res) => {
    try {
        ...
        console.error(error);
        return res.status(500).json({
            error: "No fue posible consultar hoy.",
            detalle: error.message,
        });
    }
});
router.get("/todas/ayer", async (_req, res) => {
    try {
        const ayer = obtenerFechaActualMexico();
        ayer.setDate(ayer.getDate() - 1);
        const { inicio, fin, } = crearRangoDia(ayer);
        const inicioMedicion = Date.now();
        const resultado = await calcularConsolidado(inicio, fin);
        return res.json({
            fuente: "postgresql",
            periodo: "ayer",
            fecha: formatoMexico(ayer),
            zona_horaria: ZONA_HORARIA,
            tiempo_consulta_ms: Date.now() -
                inicioMedicion,
            ...resultado,
        ...
        return res.status(500).json({
            error: "No fue posible calcular el consolidado de ayer.",
            detalle: error.message,
        });
    }
});
...
router.get("/:tienda/ayer", async (req, res) => {
    try {
        ...
        console.error(error);
        return res.status(500).json({
            error: "No fue posible consultar ayer.",
            detalle: error.message,
        });
    }
});
router.get("/todas/semana", async (_req, res) => {
    try {
        const hoy = obtenerFechaActualMexico();
        const diaSemana = hoy.getDay();
        const diasDesdeLunes = diaSemana === 0
        ...
        return res.status(500).json({
            error: "No fue posible calcular el consolidado semanal.",
            detalle: error.message,
        });
    }
});
...
router.get("/todas/mes", async (_req, res) => {
    try {
        const hoy = obtenerFechaActualMexico();
        const inicioMes = new Date(hoy.getFullYear(), hoy.getMonth(), 1, 0, 0, 0, 0);
        const fin = new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate(), 23, 59, 59, 999);
        const inicioMedicion = Date.now();
        const resultado = await calcularConsolidado(inicioMes, fin);
        return res.json({
            fuente: "postgresql",
            periodo: "mes_actual",
            desde: formatoMexico(inicioMes),
        ...
        return res.status(500).json({
            error: "No fue posible calcular el consolidado mensual.",
            detalle: error.message,
        });
    }
});
...
router.get("/:tienda/mes", async (req, res) => {
    try {
        ...
        const fin = new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate(), 23, 59, 59, 999);
        const inicioMedicion = Date.now();
        const kpis = await calcularKpisDesdeDB(codigoTienda, inicioMes, fin);
        const tiempoMs = Date.now() -
            inicioMedicion;
        return res.json({
            fuente: "postgresql",
...
//# sourceMappingURL=db-kpi.routes.js.map