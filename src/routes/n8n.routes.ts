 ...
 */
function requiereSeleccionTienda(
  texto: string
): boolean {
  const mensaje =
    normalizarTexto(texto);
 ...
 */
function convertirSeleccionMenu(
  mensaje: string
): string {
  const texto =
    ...
    try {
      ...
      if (
        ...
        return res.json({
          ...
          respuesta:
            "No fue posible identificar el número de teléfono.",
        });
      }
      ...
      return res.json({
        ...
        respuesta:
          "En este momento no fue posible consultar la información de Bafar. Intenta nuevamente.",
