    ...
    try {
        ...
        if (!telefono ||
            ...
            return res.status(400).json({
                error: "Debes enviar el teléfono del usuario.",
                ejemplo: {
         ...
         */
        const respuesta = await procesarConIA(mensaje, {
            name: usuario.name,
            role: usuario.role,
        ...
        console.error("ERROR EN AI CHAT:");
        console.error(error.response?.data ||
            error.message ||
            error);
        return res.status(500).json({
            error: "No fue posible procesar la consulta con IA.",
