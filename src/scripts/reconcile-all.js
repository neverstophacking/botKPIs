...
async function main() {
    ...
    const rangoInicio = crearRangoDia(fechaInicio).inicio;
    const rangoFin = crearRangoDia(fechaFin).fin;
    console.log("");
    console.log("================================");
    console.log(" CONCILIACION WOO VS POSTGRESQL");
    console.log("================================");
    console.log(`Periodo: ${fechaInicioTexto} a ${fechaFinTexto}`);
    ...
    for (const tienda of TIENDAS) {
        ...
        console.log("--------------------------------");
        try {
            const resultado = await conciliarTienda(tienda, fechaInicioTexto, fechaFinTexto, rangoInicio, rangoFin);
            await guardarConciliacion(resultado);
            if (!resultado.correcto) {
                todasCorrectas =
                    false;
            }
            for (const [nombre, dato,] of Object.entries(resultado.comparacion)) {
                console.log(`${nombre}: ${simbolo(dato.coincide)}`);
                console.log(`  Woo: ${dato.woo}`);
                console.log(`  DB:  ${dato.db}`);
            }
            console.log("");
            ...
            todasCorrectas =
                false;
            console.error(`ERROR conciliando ${tienda}:`);
