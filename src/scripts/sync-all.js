...
async function main() {
    ...
    for (const tienda of tiendas) {
        ...
        try {
            ...
            console.log(`${tienda}: ${resultado.pedidos_sincronizados} pedidos sincronizados`);
        }
        catch (error) {
            console.error(`ERROR sincronizando ${tienda}:`);
...
main()
    .catch((error) => {
    console.error("ERROR GENERAL DE SINCRONIZACION");
    console.error(error);
    process.exit(1);
})
    .finally(async () => {
